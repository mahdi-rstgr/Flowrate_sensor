...
static void handle_start_recording() {
  bool success = start_run();
  if (success) {
    _server.send(200, "application/json", "{\"success\":true,\"message\":\"Recording started\"}");
  } else {
    String errorMsg = "";
...
static void handle_stop_recording() {
  bool success = stop_run();
  if (success) {
    _server.send(200, "application/json", "{\"success\":true,\"message\":\"Recording stopped\"}");
  } else {
    String errorMsg = "";
    if (last_error_msg[0] != '\0') {
      errorMsg = last_error_msg;
    } else {
      errorMsg = "Failed to stop recording";
    }
    String response = "{\"success\":false,\"message\":\"" + errorMsg + "\"}";
    _server.send(500, "application/json", response);
  }
...
inline void web_begin() {
  ...
  _server.on("/start", HTTP_GET, handle_start_recording);
  _server.on("/stop", HTTP_GET, handle_stop_recording);
