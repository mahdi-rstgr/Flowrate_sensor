...
const char* WIFI_SSID = "AC-IoT-Cudy-18DC-5G";
...
static const char* RECORDING_STATE_FILE = "/recording_state.txt";

// Error message buffer for HTTP responses
char last_error_msg[256] = "";

// helpers 
static inline int wrap(int i) { return (i + N10) % N10; }

static void save_recording_state() {
  File f = LittleFS.open(RECORDING_STATE_FILE, "w");
  if (f) {
    f.printf("%d\n", recording ? 1 : 0);
    f.close();
  }
...
static void load_recording_state() {
  File f = LittleFS.open(RECORDING_STATE_FILE, "r");
  if (f) {
    String line = f.readStringUntil('\n');
    recording = (line.toInt() == 1);
    f.close();
  }
}
...
// Recording 1.0 s to LittleFS (CSV Format for simplicity)
bool start_run() {
  // Check storage availability
  if (!check_storage_available()) {
    snprintf(last_error_msg, sizeof(last_error_msg), "Storage nearly full - cannot start recording");
  ...
  // Create/clear CSV data file with header
  LittleFS.remove("/data.csv");
  File f = LittleFS.open("/data.csv", "w");
  if (!f) {
    snprintf(last_error_msg, sizeof(last_error_msg), "Failed to create data.csv file - file system error");
    Serial.println("[run] Failed to create data file");
    return false;
  }
  ...
  recording   = true;
  run_start_ms = millis();
  last_record_ms = millis() - RECORD_MS;  // Allow immediate first record

  save_recording_state();
  
  last_error_msg[0] = '\0';  // Clear error on success

  Serial.println("[run] START recording (CSV format)");
  return true;
}
...
bool stop_run() {
  recording = false;
  data_ready = true; // CSV file has data
  save_recording_state();
  Serial.println("[run] STOP recording; CSV data ready");
  return true;
}
...
static void record_if_due() {
  ...
  if (n == 0) n = 1;  // Allow recording even with no buffer data (averages will be 0)
  ...
  if (f) {
    String csvLine = String(t_s, 1);
    for (int i = 0; i < NUM_SENSORS; i++) {
      if (record_mask[i]) {
        csvLine += "," + String(f_avg[i], 2) + "," + String(t_avg[i], 1);
      } else {
        csvLine += ",,";
      }
    }
...
bool stream_csv_to_client(ESP8266WebServer& server) {
  ...
  File checkFile = LittleFS.open("/data.csv", "r");
  if (checkFile) {
    checkFile.readStringUntil('\n'); // Skip header
    bool hasData = checkFile.available() > 0;
    checkFile.close();
    return hasData;
  }
  return false;
