...
class IntegratedFlowController {
        ...
        Object.keys(this.sensorData).forEach(key => {
            if (key.startsWith('s')) {
                ...
                this.updateElement(`sensor${sensorNum}Flow`, 
                    sensor.flow_1s ? sensor.flow_1s.toFixed(1) : '--');
    ...
    updateRecordingStatus() {
        // Use polled data if available, otherwise use local state
    ...
    async startRecording() {
        try {
            const response = await fetch('/api/sensors/start-recording');
            const result = await response.json();
    ...
    async stopRecording() {
        try {
            const response = await fetch('/api/sensors/stop-recording');
            const result = await response.json();
    ...
    getSensorValue(sensorKey, valueType) {
        if (this.sensorData && this.sensorData[sensorKey] && 
            ...
            return this.sensorData[sensorKey][valueType].toFixed(1);
        }
        return '--';
    }
