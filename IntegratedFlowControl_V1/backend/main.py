...
ESP8266_IP = "192.168.10.151"  # Change this to your ESP8266's IP address, like: "192.168.1.100"

# General Configuration
FRONTEND_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'frontend')

# Server binding: allow external devices on the same network to connect
# Use environment variables to override if desired.
...
def get_local_ip():
    """Return the first non-loopback IPv4 address for this host."""
    try:
        # Use a dummy socket connection to a public IP to determine the outbound interface.
...
@app.route('/api/sensors/start-recording', methods=['GET'])
def start_sensor_recording():
    """Start recording on ESP8266"""
    try:
        success, message = esp8266_handler.start_recording()
        if success:
            return jsonify({
                'success': True,
            ...
            return jsonify({
                'success': False,
                'message': message or 'Failed to start recording on ESP8266'
            }), 500
    except Exception as e:
...
@app.route('/api/sensors/stop-recording', methods=['GET'])
def stop_sensor_recording():
    """Stop recording on ESP8266"""
    try:
        success, message = esp8266_handler.stop_recording()
        if success:
            return jsonify({
                'success': True,
            ...
            return jsonify({
                'success': False,
                'message': message or 'Failed to stop recording on ESP8266'
            }), 500
    except Exception as e:
...
def main():
    ...
    print(f"Starting web server on http://{DEFAULT_HOST}:{DEFAULT_PORT}")
    if local_ip:
        print(f"Access the unified dashboard on this device at: http://{local_ip}:{DEFAULT_PORT}")
        print(f"Access from other devices on the same network at: http://{local_ip}:{DEFAULT_PORT}")
    else:
        print(f"Access the unified dashboard at: http://localhost:{DEFAULT_PORT}")
    ...
    try:
        ...
        app.run(
            host=DEFAULT_HOST,  # Allow external connections
            port=DEFAULT_PORT,
